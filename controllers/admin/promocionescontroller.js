...
const PromocionesController = {
  ...
  init: async () => {
    ...
    document
      .getElementById("input-search-promos")
      ?.addEventListener("input", PromocionesController.renderizarTabla);
    document
      .getElementById("filter-promo-status")
      ?.addEventListener("input", PromocionesController.renderizarTabla);
    document
      .getElementById("btn-nueva-promo")
      ?.addEventListener("click", PromocionesController.prepararCrear);
    ...
    document
      .getElementById("input_promo_file")
      ?.addEventListener("change", async (e) => {
        ...
        reader.onload = (ev) => {
          document.getElementById("modal_promo_img").src = ev.target.result;
        };
        reader.readAsDataURL(file);

        // hace el upload a cloude
        try {
          ...
          document.getElementById("modal_promo_img").dataset.cloudinaryUrl =
            urlCloudinary;
          ...
          PromocionesController.mostrarModalError(
            "No se pudo subir la imagen: " + error.message,
          );
          document.getElementById("btn-upload-img").disabled = false;
          document.getElementById("btn-upload-img").innerHTML =
    ...
    document
      .querySelector("#select-promo-product")
      ?.addEventListener("click", (e) => {
        if (e.target.tagName === "OPTION") {
          const producto = PromocionesController.productos.find(
          ...
          Array.from(e.currentTarget.options).forEach((opt) => {
            opt.selected = PromocionesController.productosListados.some(
              (p) => p.id == opt.value,
            );
          });
        }
    ...
    document
      .getElementById("form-gestion-promo")
      ?.addEventListener("submit", PromocionesController.guardarPromocion);
  },
  cargarDatos: async () => {
    const promosResp = await PromocionesModel.obtenerPromociones();
    if (promosResp.status === false) {
      return PromocionesController.mostrarModalError(
        promosResp.message || "Error al cargar promociones",
      );
    }
  ...
  cargarOpcionesSelect: () => {
    ...
    select.innerHTML = PromocionesController.productos
  ...
  listarProducto: (producto) => {
    ...
    const index = PromocionesController.productosListados.findIndex(
      (x) => x.id == producto.id,
    );
  ...
  editar: (id) => {
    ...
    document.getElementById("modal_promo_desde").value = p.start_date.slice(
      0,
      16,
    );
    document.getElementById("modal_promo_hasta").value = p.end_date.slice(
      0,
      16,
    );
  ...
  guardarPromocion: async (e) => {
    ...
    try {
      ...
      PromocionesController.mostrarModalError(
        error.message || "Error al guardar promoción",
      );
    } finally {
...
<div class="modal fade" id="errorModal" tabindex="-1" aria-labelledby="errorModalLabel" aria-hidden="true">
  ...
  modificarPromocion: async (promocion) => {
    ...
    if (resultado.status === false) {
      PromocionesController.mostrarModalError(
        resultado.message || "Error al modificar promoción",
      );
      return false;
    }
    bootstrap.Modal.getInstance(
      document.getElementById("modalPromocion"),
    ).hide();
    PromocionesController.mostrarAlerta(
  ...
  crearNuevaPromocion: async (promocion) => {
    if (!promocion.name || !promocion.type) {
      PromocionesController.mostrarModalError(
        "Nombre y tipo de promoción son obligatorios",
      );
      return false;
    }
    ...
    if (resultado.status === false) {
      PromocionesController.mostrarModalError(
        resultado.message || "Error al crear promoción",
      );
      return false;
    }
    bootstrap.Modal.getInstance(
  ...
  eliminarPromo: async (id) => {
    if (
      !confirm(`¿Está seguro de que desea eliminar esta promocion? Id: ${id}`)
    ) {
      return;
    }
    try {
      ...
      if (resultado.status === false) {
        PromocionesController.mostrarModalError(
          resultado.message || "Error al eliminar promoción",
        );
        return;
      }
      PromocionesController.mostrarAlerta(
