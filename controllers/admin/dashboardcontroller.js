...
const DashboardController = {
  ...
  init: async () => {
    ...
    try {
      const [dashboard, orders, promociones, fiveOrders] = await Promise.all([
      ...
      document
        .querySelector("#select-date-orders")
        ?.addEventListener("change", async function (e) {
          const data = await DashboardController.getOrders(this.value);
          if (data.status === false) {
            return DashboardController.mostrarModalError(
              data.message || "Error al cargar pedidos",
            );
          }
      ...
      document
        .querySelector("#select-type-barchart")
        ?.addEventListener("change", async function (e) {
          if (this.value == "productos") {
            DashboardController.initChartRanking(
              dashboard.data.products,
              document.querySelector("#select-date-combined").value,
            );
          } else {
            DashboardController.initChartRanking(
      ...
      document
        .querySelector("#select-date-combined")
        ?.addEventListener("change", async function (e) {
          if (
            document.querySelector("#select-type-barchart").value == "productos"
          ) {
            DashboardController.initChartRanking(
              dashboard.data.products,
              this.value,
      ...
      document
        .querySelector("#btn-close-session")
  ...
  getDashboard: async () => {
    ...
    const peticion = await fetch(
      "http://localhost:8000/api/admin/dashboard/index",
      {
        method: "GET",
        headers,
      },
  ...
  getOrders: async (date) => {
    ...
    const peticion = await fetch(
      `http://localhost:8000/api/admin/dashboard/orders?range=${date}`,
      {
        method: "GET",
        headers,
      },
  ...
  loadSummary: (summary) => {
    document.querySelector("#today-revenue-container").innerHTML =
      "$ " + parseFloat(summary.total_revenue).toFixed(2);
    document.querySelector("#today-order-container").innerHTML =
      summary.orders_count;
    document.querySelector("#today-order-active").innerHTML =
      summary.orders_active;
    document.querySelector("#today-order-inactive").innerHTML =
      summary.orders_pendiente;
  },
